...
まだ最終定理までは進めず、次段階の honest interface を固定する。
-/
...
/-- Once block-body closure is available, block-statement closure should be derived by
open/body/close assembly rather than by re-running `(.block ss)` under an opened state. -/
axiom block_function_body_closure_concrete_refined_honest
    {Γ : TypeEnv} {σ : State} {ss : StmtBlock} :
    BodyReadyConcrete Γ σ (.block ss) →
