...
/-!
# Closure.Transitions.Major.CloseScopeTopFrameExtension

`closeScope_preserves_concrete_state_via_decomposition` は
`pushTypeScope Γ` から `Γ` への base case を扱う。

このファイルではそれを block 本線で必要な一般形

`TopFrameExtensionOf Γ Θ -> ScopedTypedStateConcrete Θ σ -> CloseScope σ σ' -> ScopedTypedStateConcrete Γ σ'`

へ持ち上げる。

数学的には、close-scope 後の状態は pre-state の lower-fragment restriction である。
C++ 的には、block 退出時には top scope の automatic objects だけが寿命を終え、
外側 scope の型・名前・binding・ownership はそのまま残る。
-/
