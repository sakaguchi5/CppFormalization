...
private theorem heapLiveTypedAt_assigns_preserved
  ...
  · subst ha
    ...
    subst c
    -- ⟨c0, hheap0, hty, halive⟩ が元の hlive の中身に相当します
  ...
  refine
    { frameDepth := by
      ...
      refsNotOwned := by
        ...
        -- ★ ここが重要：fr と fr_old が同じであることを証明して subst する
        have hfr_eq : fr = fr_old := by
          -- writeHeap は scopes を変えないことを利用
          simp [writeHeap] at hk
          rw [hk] at hk_old
          injection hk_old
        subst hfr_eq

        -- これで fr_old が fr に置き換わり、href がそのまま使えます
      ...
      nextFresh := by
        ...
        · have hne : a0 ≠ σ.next := by
          ...
          -- writeHeap 後の a0 以外のアドレス（σ.next）が none であることを示す
          simpa [writeHeap, hne, eq_comm] using hheapNone
        · intro k fr hk
          -- scopes は writeHeap で変化しないため、そのまま simpa で通る
