...
/-!
# CppFormalization.Cpp3.Soundness.Derive.BoundaryConstruction.Flow

Bottom-up constructors for selected post-state boundaries and their stability
packages.

`BoundaryConstruction.Core` builds entry boundaries for the current program point.
This file handles the next layer: once a selected semantic route is known, Effects
and SafetyFragment provide the post-state boundary surface, and Stability packages
that surface as the theorem-facing preservation object.
-/
...
/-- Source for the boundary available after the head of a sequence completes
normally. -/
...
/-- Source for the boundary available after a block head completes normally. -/
...
/-- Source for an if-statement selected-branch boundary. -/
inductive SelectedBranchBoundarySource
  ...
  | thenBoundary
      ...
      SelectedBranchBoundarySource Γ Γc σ σc cond thenBranch elseBranch
        .thenBranch
  | elseBoundary
      (source : Boundary.StmtBoundary Γ σ (.ite cond thenBranch elseBranch))
      (route : Semantics.SelectedBranchRoute σ σc cond thenBranch elseBranch
...
/-- Source for a selected while boundary route. -/
...
/-- Build the routed opened-block boundary used by `Boundary.Flow`. -/
